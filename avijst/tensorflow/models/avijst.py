...
class AVIJST(object):
    ...
    def __init__(self, network_architecture, transfer_fct=tf.nn.softplus,
        ...
        self.x_pi = tf.placeholder(tf.float32, [None, network_architecture["n_input_pi"]])
        self.y = tf.placeholder(tf.float32, [None, network_architecture["n_p"]])
    ...
    def _create_network(self, cls_model):
        ...
        with tf.name_scope('z_sample'):
            eps = tf.random_normal((self.batch_size * self.pol_dim, self.h_dim), 0, 1,
                                   dtype=tf.float32)
        ...
        with tf.name_scope('pi_sample'):
            pi_eps = tf.random_normal((self.batch_size, self.pol_dim), 0, 1,
                                   dtype=tf.float32)
            self.pi = tf.add(self.pi_mean,
                            tf.multiply(tf.sqrt(tf.exp(self.pi_log_sigma_sq)), pi_eps))
            self.pi_sigma = tf.exp(self.pi_log_sigma_sq)
    ...
    def _initialize_weights(self, n_hidden_recog_1, n_hidden_recog_2,
        ...
        all_weights['weights_recog'] = {
            ...
            'h2': tf.get_variable('h2',[n_hidden_recog_1, n_hidden_recog_2],
               initializer=tf.contrib.layers.xavier_initializer()),
            'pi_h1': tf.get_variable('pi_h1',[n_input_pi, n_hidden_recog_1],
               initializer=tf.contrib.layers.xavier_initializer()),
            'pi_h2': tf.get_variable('pi_h2',[n_hidden_recog_1, n_hidden_recog_2],
               initializer=tf.contrib.layers.xavier_initializer()),
            'out_mean': tf.get_variable('out_mean',[n_hidden_recog_2, self.mix_dim],
               initializer=tf.contrib.layers.xavier_initializer()),
            'out_log_sigma': tf.get_variable('out_log_sigma',[n_hidden_recog_2, self.mix_dim],
               initializer=tf.contrib.layers.xavier_initializer()),
            'pi_out_mean': tf.get_variable('pi_out_mean',[n_hidden_recog_2, self.pol_dim],
               initializer=tf.contrib.layers.xavier_initializer()),
            'pi_out_log_sigma': tf.get_variable('pi_out_log_sigma',[n_hidden_recog_2, self.pol_dim],
               initializer=tf.contrib.layers.xavier_initializer()),
            }
        all_weights['biases_recog'] = {
            'b1': tf.Variable(tf.zeros([n_hidden_recog_1], dtype=tf.float32), name='b1'),
    ...
    def _recognition_network(self, weights, biases, cls_model=None):
        ...
        with tf.name_scope('pi_layers'):
            ...
            pi_layer_do = tf.nn.dropout(pi_layer, self.keep_prob)#Dropout(0.4)(pi_layer)
    ...
    def _create_loss_optimizer(self):
        ...
        latent_z_loss = 0.5*( tf.reduce_sum(tf.div(self.sigma,self.var2),-1)+\
        ...
        latent_s_loss = 0.5*( tf.reduce_sum(tf.div(self.pi_sigma,self.pi_var2),1)+\
        tf.reduce_sum( tf.multiply(tf.div((self.pi_mu2 - self.pi_mean),self.pi_var2),
                  (self.pi_mu2 - self.pi_mean)),1) - self.pol_dim +\
                           tf.reduce_sum(tf.log(self.pi_var2),1)  - tf.reduce_sum(self.pi_log_sigma_sq  ,1) )
    ...
    def _create_cls_loss_optimizer(self):
        self.logits = tf.nn.softmax(self.pi_mean)
        self.cls_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
            logits=self.logits, labels=self.y))
        #self.cls_loss = tf.reduce_mean(categorical_crossentropy(self.y, self.logits))
        self.cls_optimizer = tf.train.AdamOptimizer(learning_rate=self.cls_learning_rate).minimize(self.cls_loss)
